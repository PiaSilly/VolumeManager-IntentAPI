...
class Manager(private val context: Context, private val dataStore: DataStore<Preferences>) {
    ...
    /**
     * Sets the per-app playback volume multiplier.
     *
     * @param packageName Android package name, e.g. "com.spotify.music"
     * @param value multiplier in the range 0.0..1.0
     * @return true if the package exists and the value was stored/applied
     */
    fun setAppVolume(packageName: String, value: Float): Boolean {
        val normalizedPackage = packageName.trim()
        if (normalizedPackage.isEmpty() || value.isNaN() || value.isInfinite()) {
            return false
        }
        ...
        val app = apps.getOrPut(normalizedPackage) {
            App(
                normalizedPackage,
        ...
        app.volume = value.coerceIn(0f, 1f)
        return true
    }
